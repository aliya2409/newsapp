...
public class NewsFeatureStepDefs {
    ...
    private final String ENDPOINT_DELETE_NEWS_LIST = "/news/delete";
    ...
    public void setIds() throws JsonProcessingException {
        List<Long> ids = new ArrayList<>();
        ids.add(1L);
        IdsDTO idsDTO = new IdsDTO(ids);
        ObjectMapper objectMapper = new ObjectMapper();
        json = objectMapper.writeValueAsString(idsDTO);
    }

    @When("^a user makes POST request to /news/delete$")
    public void postDeleteList() throws Exception {
        response = mockMvc.perform(post(ENDPOINT_DELETE_NEWS_LIST).contentType(APPLICATION_JSON_UTF8).content(json));
    }
}
